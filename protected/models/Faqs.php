...
class Faqs extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('question', 'required'),
                        array('attachment', 'file', 'types'=>'jpg,pdf'),
			array('question, answer, attachment', 'length', 'max'=>255),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, question, answer, attachment', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
        ...
        public function downloadAttachment($attachment){      
          if($attachment){
               return  '<a target="_blank" href="'.Yii::app()->getBaseUrl().'/userfiles/faq_attachments/'.$attachment.'">Download</a>'; 
            }
